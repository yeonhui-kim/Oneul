...
function infiniteScrollWith(requestUrl, containerClass, returningHTML,query) {
    ...
    // window.oncscroll 에넣어줄 callback 함수를 정의해준다 (스크롤값이 바뀔때마다실행되는 event함수)
    let infiniteScrollCallBack = () => {

        if (Math.floor(window.pageYOffset) > scrollable-200) {

            // 로딩중에는 이벤트막아놈
            window.removeEventListener("scroll", infiniteScrollCallBack);
            ...
            fetch(`${requestUrl}/${count}?q=${query}`).then((response) => response.json(), ()=>{
                ...
                window.addEventListener("scroll", infiniteScrollCallBack);
